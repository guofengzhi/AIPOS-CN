...
(function ($, window, document, undefined) {
    ...
    var BaseForm = function (element, options) {
        ...
        this.arraySelectorElement = "[data-flag='arraySelector']";
    ...
    BaseForm.prototype.initDatePicker = function () {
        ...
        if (form.find('[data-flag="datepicker"]').length > 0) {
            form.find('[data-flag="datepicker"]').datepicker({
                autoclose: true,
                todayHighlight : true,
                endDate : $(this).attr("endDate")?$(this).attr("endDate"):new Date(),
     ...
     * 外部数组数据控件
     */
    BaseForm.prototype.initArraySelector=function(arraySelectorElement){
        var _this=this;
        var element=arraySelectorElement?arraySelectorElement:this.arraySelectorElement;
        var elements=this.$element.find(element);
        $(elements).each(function(index,item){
            var data=$(item).data("datasrc");
            var autoload=$(item).data("autoload")=="false"?false:true;
            if(data){
                if(autoload){
                    if($(item).is("input"))
                        _this.buildArrayBox(this,data);
                    else if($(item).is("select"))
                        _this.buildArraySelect(this,data);
                }else{
                    var that=this;
                    $(this).click(function(){
                        _this.buildArraySelect(that,data);
                    })
                }
            }
        });
  ...
  //数据来源为数组的 radio checkbox
    BaseForm.prototype.buildArrayBox=function (selector,data) {
        var builder=this.buildAjaxBox(selector);
        $dataSource.getArrayData(data,builder);
    }
    //数据来源为字典的下拉框
  ...
  //数据来源为url的下拉框
    BaseForm.prototype.buildArraySelect=function(selector,data){
        var builder=this.buildAjaxSelector(selector);
        $dataSource.getDataByArray(data, builder);
    }
     ...
     * 表单属性只读
     */
    BaseForm.prototype.formReadonly=function(){
    	var form = this.$element;
    	if(form.length == 0)
    		return;
    	form.find(':input[name]').attr("disabled","disabled");
    	form.find(".box-footer").hide();
    	//选择图标按钮只读
    	form.find("#selectIcon").addClass("disabled");
    	form.find("button").addClass("disabled");
    	form.find("span").addClass("disabled");
		//还原新增、编辑、删除按钮样式
    	form.find(".box-header button").removeClass("btn-primary").addClass("btn-default");
		//还原校验框
     ...
     * 表单属性写属性
     */
    BaseForm.prototype.formWritable=function(action){
    	var form = this.$element;
    	if(form.length == 0)
    		return;
    	form.find("input[name]").removeAttr("disabled");
    	form.find('textarea[name]').removeAttr("disabled");
    	form.find(".box-footer").show();
    	form.find("#selectIcon").removeClass("disabled");
    	form.find("button").removeClass("disabled");
    	form.find("span").removeClass("disabled");
    	form.find("select2").removeAttr("disabled").removeClass("disabled")
    	form.find("select2-select").removeAttr("disabled").removeClass("disabled")
    	form.find(".box-header button").removeClass("btn-primary").addClass("btn-default");
		if(action)
			form.find(".box-header button[data-btn-type='"+action+"']").removeClass("btn-default").addClass("btn-primary");
    }
