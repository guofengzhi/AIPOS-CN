...
import com.jiewen.modules.sys.web.LoginController;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
        ...
        if(null !=UserUtils.getSession().getAttribute(token.getUsername()) && 
        		!"".equals(UserUtils.getSession().getAttribute(token.getUsername()))){
			int loginCount=(int) UserUtils.getSession().getAttribute(token.getUsername());
		   if(loginCount > 2){
			// 校验登录验证码
		        if (LoginController.isValidateCodeLogin(token.getUsername(), false, false, errorTimes)) {
		            Session session = UserUtils.getSession();
		            String code = (String) session.getAttribute(ValidateCodeServlet.VALIDATE_CODE);
		            if (token.getCaptcha() == null || !token.getCaptcha().toUpperCase().equals(code)) {
		                throw new AuthenticationException("msg:"+ messageSourceUtil.getMessage("common.VerificationError"));
		            }
		        }
		   }
		}
        ...
        if (user != null) {
        	//验证用户是否是开发者
        	if(StringUtils.equals(Global.NO, user.getIsDeveloper())){
        		 throw new AuthenticationException("msg:"+ 
        		    messageSourceUtil.getMessage("non.developers.are.not.allowed.to.log.in"));
        	}
        	// 校验用户名密码
            if (Global.NO.equals(user.getLoginFlag())) {
                throw new AuthenticationException("msg:"+ 
                	messageSourceUtil.getMessage("common.TheAccountIsNotAllowedToLoginOn"));
            }
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        if (!Global.TRUE.equals(Global.getConfig("user.multiAccountLogin"))) {
            ...
            if (!sessions.isEmpty()) {
                ...
                // 记住我进来的，并且当前用户已登录，则退出当前用户提示信息。
                    ...
                    throw new AuthenticationException("msg:"+ 
                    messageSourceUtil.getMessage("common.AccountHasBeenLogin"));
                }
            }
        }
        ...
        if (user != null) {
            ...
            List<Menu> list = UserUtils.getMenuList(LocaleContextHolder.getLocale().toString());
            for (Menu menu : list) {
            ...
            // 记录登录日志
            String sysLoginLog = messageSourceUtil.getMessage("sys.login.log.sysLong");
            LogUtils.saveLog(ServletsUtils.getRequest(), sysLoginLog);
            return info;
        } else {
            return null;
        }
