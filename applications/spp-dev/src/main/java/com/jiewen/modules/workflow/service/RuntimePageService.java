...
public class RuntimePageService {

    private static final Logger logger = LoggerFactory
            .getLogger(RuntimePageService.class);
    ...
    public PageInfo<ActivityVo> getActivityList(ProcessInstanceVo procInstVo) {
        String processInstanceId = null;
        if (procInstVo != null) {
            if (StringUtils.isNotEmpty(procInstVo.getInstanceId())) {
                processInstanceId = procInstVo.getInstanceId();
            }
        }
        // 已执行的流程节点
        ...
        for (HistoricActivityInstance historicActivityInstance : historicActivityInstanceList) {
            ...
            if (historicActivityInstance.getActivityType().equals("startEvent")) {
                vo.setAssigneeName(
                        identityPageService.getUserNamesByUserIds(processInstance == null
                                ? startUserId : getStartUserId(processInstance)));
            } else {
                if (!StringUtils.isEmpty(vo.getAssignee())) {
                    vo.setAssigneeName(
                            identityPageService.getUserNamesByUserIds(vo.getAssignee()));
                } else {
                    // 未指定审批审批人获取
                    vo.setAssigneeName(getCandidateUserNames(
                            getActivity(historicActivityInstance, activityList),
                            processInstanceId));
                }
            }
            // 节点状态
        ...
        /*
         * for (ActivityImpl activity : activityList) { for (String activeId : activeIds) { if
         * (activity.getId().equals(activeId)) { nextActivities.add(activity); break; } } }
         */
        ...
        /*
         * for (ActivityImpl nextActivity : nextActivities) { List<PvmTransition> transitions =
         * nextActivity.getOutgoingTransitions(); for (PvmTransition transition : transitions) {
         * List<PvmActivity> activities = findNextUserTask(transition); if (!activities.isEmpty()) {
         * for (PvmActivity activity : activities) { if
         * (findInDoneActivityList(historicActivityInstanceList, activity)) { ActivityVo vo = new
         * ActivityVo(); vo.setId(activity.getId()); vo.setActivityId(activity.getId());
         ...
         * vo.setActivityState(Constants.STATE_TODO); voList.add(vo); curActivity = activity; } } }
         * } }
         */
        ...
        // 尚未执行的流程节点
        /*
         * if (processInstance != null) { for (ActivityImpl activity : activityList) { boolean done
         * = false; for (ActivityVo activityVo : voList) { if
         * (activity.getId().equals(activityVo.getActivityId())) { done = true; break; } } if (!done
         * && activity.getProperty("type").equals("userTask")) { ActivityVo vo = new ActivityVo();
         * vo.setId(activity.getId()); vo.setActivityName(activity.getProperty("name").toString());
         * vo.setAssigneeName(getCandidateUserNames(activity, processInstanceId));
         * vo.setActivityState(Constants.STATE_TODO); voList.add(vo); } } }
         */
    ...
    public String getStartUserId(ProcessInstance processInstance) {
        ...
        String assign = runtimeService
                .getVariable(processInstance.getProcessInstanceId(), initiator)
                .toString();
        return assign;
    }

    /**
     * 获取尚未执行的节点的可能执行人姓名，以逗号分隔 需要使用命令模式动态执行表达式 不然报lazy load expression out of activiti异常
     *
     * @param activity
     *            流程定义的用户活动节点
                ...
                .execute(new Command<String>() {
                    ...
                    public String execute(CommandContext commandContext) {
                        ...
                        ExecutionEntity execution = (ExecutionEntity) runtimeService
                                ...
                                .processInstanceId(processInstanceId).singleResult();
                        TaskDefinition taskDefinition = (TaskDefinition) activity
                        ...
                        if (taskDefinition.getAssigneeExpression() != null) {
                            try {
                                ...
                                logger.error(messageSourceUtil
                                        .getMessage("workflow.get.people.treatment.error")
                                        + ex.getMessage());
                                assignee = null;
                            }
                            retNames = assignee != null
                                    ? identityPageService.getUserNamesByUserIds(assignee)
                                    : Constants.INDETERMINATE;
                            return retNames;
                        }
                        // 委托人,同受理人同一人的情况下不显示
                        if (taskDefinition.getOwnerExpression() != null) {
                            try {
                                String owner = ((String) taskDefinition
                                        .getOwnerExpression().getValue(execution));
                            } catch (Exception ex) {
                                logger.error(messageSourceUtil
                                        .getMessage("workflow.get.people.principal.error")
                                        + ex.getMessage());
                            }
                        }
                        ...
                        if (!taskDefinition.getCandidateGroupIdExpressions().isEmpty()) {
                            ...
                            for (Expression groupIdExpr : taskDefinition
                                ...
                                try {
                                    ...
                                    logger.error(messageSourceUtil
                                            .getMessage("workflow.get.candidate.groups")
                                            + ex.getMessage());
                                    value = null;
                                }
                                if (value != null) {
                            ...
                            if (!groupIdList.isEmpty()) {
                                ...
                                return identityPageService.getUserNamesByGroupIds(
                                        StringUtils.join(groupIdArr, ','));
                            } else {
                                return Constants.INDETERMINATE;
                            }

                        } else if (!taskDefinition.getCandidateUserIdExpressions()
                                .isEmpty()) {
                            ...
                            for (Expression userIdExpr : taskDefinition
                                ...
                                try {
                                    ...
                                    logger.error(messageSourceUtil.getMessage(
                                            "workflow.get.candidate.people.error")
                                            + ex.getMessage());
                                    value = null;
                                }
                                if (value != null) {
                            ...
                            if (!userIdList.isEmpty()) {
                                ...
                                return identityPageService.getUserNamesByUserIds(
                                        StringUtils.join(userIdArr, ','));
                            } else {
                                return Constants.INDETERMINATE;
                            }
                        }
    ...
    public ActivityImpl getActivity(HistoricActivityInstance activityInstance,
    ...
    public void findNextActivity(List<ActivityVo> voList, String processInstanceId,
        ...
        for (PvmTransition nextTran : nextTrans) {
            ...
            if ("userTask".equals(activity.getProperty("type").toString())) {
                if (flowName != null && isInApprovedText(Constants.APPROVED_PASSED,
                        flowName.toString())) {
                    ActivityVo vo = new ActivityVo();
                    ...
                    findNextActivity(voList, processInstanceId, activity);
                        ...
                        || !isInApprovedText(Constants.APPROVED_REJECT,
                                flowName.toString())) {
                    // 条件路径
                    Object conditionText = nextTran.getProperty("conditionText");
                    if (conditionText != null) {
                        boolean targetTask = isTargetTask(conditionText.toString(),
                                processInstanceId, nextTran);
                        if (targetTask) {
                            ActivityVo vo = new ActivityVo();
                            vo.setId(activity.getId());
                            vo.setActivityName(activity.getProperty("name").toString());
                            vo.setAssigneeName(getCandidateUserNames(
                                    (ActivityImpl) activity, processInstanceId));
                            vo.setActivityState(Constants.STATE_TODO);
                            voList.add(vo);
                            findNextActivity(voList, processInstanceId, activity);
                        }
    ...
    public boolean isTargetTask(final String expressionText, String processInstanceId,
        ...
        final ExecutionEntity execution = (ExecutionEntity) runtimeService
                .createExecutionQuery().executionId(processInstanceId).singleResult();
        Boolean result = ((RuntimeServiceImpl) runtimeService).getCommandExecutor()
                .execute(new Command<Boolean>() {

                    @Override
                    public Boolean execute(CommandContext commandContext) {
                        UelExpressionCondition flowCondition = (UelExpressionCondition) transition
                                .getProperty("condition");
                        boolean evel_ret = flowCondition.evaluate(transition.getId(),
                                execution);
                        return evel_ret;
                    }
                });
        ...
        /*
         * ExpressionFactory factory = new ExpressionFactoryImpl(); SimpleContext context = new
         * SimpleContext(); Map<String, Object> variables =
         * runtimeService.getVariables(processInstanceId); for (String key : variables.keySet()) {
         * context.setVariable(key, factory.createValueExpression(variables.get(key),
         * variables.get(key).getClass())); } try { ValueExpression e =
         * factory.createValueExpression(context, expressionText, boolean.class); return (boolean)
         * e.getValue(context); } catch (Exception ex) { return false; }
         */
    }
     ...
     */
    public Result startProcessInstanceByKey(String processDefinitionKey, String name,
                                            Map<String, Object> variables, String userId,
                                            String businessKey) {
        // 校验流程定义是否存在
        final ProcessDefinitionEntity processDefinitionEntity = (ProcessDefinitionEntity) repositoryService
                .createProcessDefinitionQuery().processDefinitionKey(processDefinitionKey)
                .latestVersion().active().singleResult();
        if (processDefinitionEntity == null) {
            return ResultGenerator.genFailResult(
                    messageSourceUtil.getMessage("activity.demo.instance.start.error")
                            + "'" + processDefinitionKey + "'" + messageSourceUtil
                                    .getMessage("activity.demo.instance.not.found"));
        }
        // 设置流程启动人
        ...
        try {
            ...
            Object retObj = ((FormServiceImpl) formService).getCommandExecutor()
                    .execute(new Command<Object>() {
                        ...
                        public Object execute(CommandContext commandContext) {
                            ...
                            DefaultStartFormHandler startFormHandler = (DefaultStartFormHandler) processDefinition
                                    .getStartFormHandler();
                            for (FormPropertyHandler formPropertyHandler : startFormHandler
            ...
            if (startFormData != null) {
                ...
                for (FormProperty property : properties) {
                    if (!variables.containsKey(property.getId())) {
                        variables.put(property.getId(),
                                this.getVariable(property, property.getValue()));
                    }
                }
            }
            ...
            return ResultGenerator.genFailResult(messageSourceUtil.getMessage(
                    "activity.demo.instance.start.error.result") + ex.getMessage());
        }
        runtimeService.setProcessInstanceName(processInstance.getId(), name);
        return ResultGenerator.genSuccessResult(
                messageSourceUtil.getMessage("activity.demo.start.success"));
    }

    public Object getVariable(FormProperty formProperty, String result) {
        String value;
        value = StringUtils.isEmpty(result) ? formProperty.getValue() : result;
