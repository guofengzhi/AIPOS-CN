...
public class CustomProcessDiagramGenerator extends DefaultProcessDiagramGenerator {
        ...
        if (drawInstruction != null) {
            ...
            if (scaleFactor == 1.0) {
                ...
                processDiagramCanvas.drawActivityMarkers((int) graphicInfo.getX(),
                        ...
                        (int) graphicInfo.getHeight(), multiInstanceSequential,
    ...
    protected static GraphicInfo getCustomLineCenter(List<GraphicInfo> graphicInfoList) {
        ...
        for (int i = 1; i < graphicInfoList.size(); i++) {
            ...
            length += Math.sqrt(
                    Math.pow((int) graphicInfo.getX() - (int) previousGraphicInfo.getX(), 2) + Math
                            .pow((int) graphicInfo.getY() - (int) previousGraphicInfo.getY(), 2));
            lengths[i] = length;
        }
        m = length / 2;
        int p1 = 0;
