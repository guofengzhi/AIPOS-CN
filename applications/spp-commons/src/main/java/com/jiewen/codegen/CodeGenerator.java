...
import com.jiewen.constant.ProjectConstant;
...
public class CodeGenerator {
    ...
    // JDBC配置，请修改为你项目的实际配置
    private static final String JDBC_URL = "jdbc:mysql://localhost:3306/posota";
    ...
    private static final String TEMPLATE_FILE_PATH = PROJECT_PATH
            + "/src/main/resources/generator/template"; // 模板位置
    ...
    private static final String PACKAGE_PATH_SERVICE = packageConvertPath(
            ProjectConstant.SERVICE_PACKAGE); // 生成的Service存放路径

    private static final String PACKAGE_PATH_SERVICE_IMPL = packageConvertPath(
            ProjectConstant.SERVICE_IMPL_PACKAGE); // 生成的Service实现存放路径

    private static final String PACKAGE_PATH_CONTROLLER = packageConvertPath(
            ProjectConstant.CONTROLLER_PACKAGE); // 生成的Controller存放路径
...
//        Configurator.setRootLevel(Level.DEBUG);
        // genCode("输入表名");
        genCode("t_upgrade_translog", "TransLog");
    }
    ...
    public static void genModelAndMapper(String tableName, String modelName) {
        ...
        String modelNameVal = modelName;
        if (StringUtils.isEmpty(modelName)) {
            modelNameVal = tableNameConvertUpperCamel(tableName);
        }
        logger.info("{}.java 生成成功", modelNameVal);
        logger.info("{}Mapper.java 生成成功", modelNameVal);
        logger.info("{}Mapper.xml 生成成功", modelNameVal);
    }
    ...
    private static String tableNameConvertMappingPath(String tableName) {
        ...
        tableNameLowerCase = tableName.toLowerCase(); // 兼容使用大写的表名
        return "/" + (tableNameLowerCase.contains("_") ? tableNameLowerCase.replaceAll("_", "/")
                : tableNameLowerCase);
    }
