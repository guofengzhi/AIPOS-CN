...
<script type="text/javascript">
	// 百度地图API功能
	var map = new BMap.Map("bmapInfo");    // 创建Map实例	
	var deviceLocation =new BMap.Point('${location.longitude }','${location.latitude }');	
	
	var bmapLoad=1;
	$('a[data-toggle="tab"]').on("shown.bs.tab", function (e) {
		if($("#bmapInfo").hasClass("active")){
			if(bmapLoad == 1){
				map.centerAndZoom(deviceLocation, 14);  // 初始化地图,设置中心点坐标和地图级别
				
				map.addEventListener("tilesloaded",function(){
					map.panTo(deviceLocation);
					map.enableScrollWheelZoom(true);
					var marker = new BMap.Marker(deviceLocation);	
					map.addOverlay(marker);
					var label = new BMap.Label('${location.addr}',{offset:new BMap.Size(0, 25)});
					marker.setLabel(label);
					});
			}
			bmapLoad = bmapLoad + 1;
			map.addEventListener("tilesloaded",function(){map.panTo(deviceLocation);});
					...
					<div class="tab-content no-padding">
						...
						<div style="width: 50%;float: left;">
							...
							<span class="span_line">
								<c:if test="${totalMemery >= 1073741824 }">
									 <fmt:formatNumber type="number" value="${deviceInfo.totalmemory/1073741824}" maxFractionDigits="2"/>G
								</c:if>
								<c:if test="${totalMemery < 1073741824}">
									<fmt:formatNumber type="number" value="${deviceInfo.totalmemory/1048576}" maxFractionDigits="2"/>M
								</c:if>
							
							</span>
							<br /> <span class="span_line">&nbsp;&nbsp;&nbsp;<spring:message code="modules.device.deviceinfo.availmemory"/>：</span>
							<span class="span_line">
							<c:if test="${deviceInfo.availmemory >= 1073741824 }">
									 <fmt:formatNumber type="number" value="${deviceInfo.availmemory/1073741824}" maxFractionDigits="2"/>G
								</c:if>
								<c:if test="${deviceInfo.availmemory < 1073741824}">
									<fmt:formatNumber type="number" value="${deviceInfo.availmemory/1048576}" maxFractionDigits="2"/>M
								</c:if>
							
							</span>		
							<br /> <span class="span_line">&nbsp;&nbsp;&nbsp;<spring:message code="modules.device.deviceinfo.battery"/>：</span>
							<span class="span_line">${deviceInfo.battery}</span>	
