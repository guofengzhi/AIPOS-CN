...
<spring:message code="common.delete" var="commondelete"/>
...
<div class="modal-header modalsbg">
	...
	<h5 class="modal-title"><spring:message code="modules.device.batch.adding.device"/></h5>
</div>


<div class="modal-body">
	<div class="box-body">
	 <button class="btn btn-info btn lg text-center" id="downLoadMerTemp"><spring:message code="modules.device.download.the.device.information.template"/></button>
	   <div></div>
...
<script>
	$("#downLoadMerTemp").click(function() {
		$.download(basePath + "/device/download?type=device", "post", "");
	})
	$(function(){
		  var $ = jQuery,
	        $list = $('#thelist'),
		   ...
		   uploader = WebUploader.create({
		    ...
		    // 文件接收服务端。
		    server: basePath + "/device/addDeviceInfo",
		...
		uploader.on( 'fileQueued', function( file ) {
	        ...
	        '<button class="close" title="'+commondelete+'"><li class="fa fa-remove"></li></button> <h4 class="info">' + file.name + '</h4>' +
		...
		uploader.on( 'uploadError', function( file, response ) {
		    $( '#'+file.id ).find('p.state').text('<spring:message code="modules.device.upload.error.please.check.the.file.is.correct"/>');
		});
