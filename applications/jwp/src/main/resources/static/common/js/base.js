    ...
    jQuery.ajax({
		...
		success : function(data, status) {
			...
			if(data&&data.code&&data.code=='101'){
				modals.error({title: '<h2 class="headline text-red">'+data.code+'</h2>', text:jQuery.i18n.prop("common.sys.tips")+data.message});
				return false;
			}
			if (callback) { 
...
function ajaxPostWithDeferred(url, params, callback) {
    ...
    jQuery.ajax({
		...
		success : function(data, status) {
			result = data;
			if(data&&data.code&&data.code=='101'){
				modals.error(jQuery.i18n.prop("common.sys.tips")+data.message);
				return false;
			}
			defer.resolve(data);
			/*
			 * if (callback) { callback.call(this, data, status); }
			 */
		},
		error : function(err, err1, err2) {
			console.log("ajaxPost发生异常，请仔细检查请求url是否正确");
