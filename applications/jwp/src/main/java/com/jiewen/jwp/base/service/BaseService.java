...
public abstract class BaseService {
                ...
                for (String oa : StringUtils.split(officeAlias, ",")) {
                    if (!dataScope.contains(r.getDataScope()) && StringUtils.isNotBlank(oa)) {
                        if (Role.DATA_SCOPE_ALL.equals(r.getDataScope())) {
                            ...
                            sqlString.append(" OR EXISTS (SELECT 1 FROM sys_role_office WHERE role_id = '"
                                    + r.getId() + "'");
     ...
     */
    public static void dataScopeFilter(DataEntity entity, String sqlMapKey, String officeWheres,
            String userWheres) {
        ...
        // 数据范围（1：所有数据；2：所属机构及以下数据；3：所在机构数; 8：仅本人数据；9：按明细设置）
        StringBuilder sqlString = new StringBuilder();
