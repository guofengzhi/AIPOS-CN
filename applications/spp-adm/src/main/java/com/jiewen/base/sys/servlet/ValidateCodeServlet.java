...
package com.jiewen.base.sys.servlet;
 ...
 * 生成随机验证码.
 */
@WebServlet(urlPatterns = "/validateCode", description = "验证码servlet")
public class ValidateCodeServlet extends HttpServlet {
    ...
    private void createImage(HttpServletRequest request, HttpServletResponse response)
        ...
        if (StringUtils.isNumeric(width) && StringUtils.isNumeric(height)) {
            defaultWidth = NumberUtils.toInt(width);
            defaultHeight = NumberUtils.toInt(height);
        }

        BufferedImage image = new BufferedImage(defaultWidth, defaultHeight,
    ...
    private void createBackground(Graphics g) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            Random random = new Random();
            int x = random.nextInt(defaultWidth);
            int y = random.nextInt(defaultHeight);
