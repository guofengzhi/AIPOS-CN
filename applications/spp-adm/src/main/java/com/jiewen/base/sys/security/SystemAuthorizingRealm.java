...
import com.jiewen.base.sys.web.LoginController;
import com.jiewen.jwp.common.utils.Encodes;
import com.jiewen.spp.modules.customer.service.CustomerUserService;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
        ...
        User user = null;
        // 校验用户名密码
        if (StringUtils.equalsIgnoreCase(token.getLoginType(), "customer")) {
            user = customerUserService.getByLoginName(token.getUsername());
        } else {
            user = systemService.getUserByLoginName(token.getUsername());
        }
        if (user != null) {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        if (!Global.TRUE.equals(Global.getConfig("user.multiAccountLogin"))) {
            Collection<Session> sessions = systemService.getSessionDao().getActiveSessions(true,
        ...
        User user = null;
        if (StringUtils.equalsIgnoreCase(principal.getUserType(), "9")) {
            user = customerUserService.getByLoginName(principal.getLoginName());
        } else {
            user = systemService.getUserByLoginName(principal.getLoginName());
        }
        if (user != null) {
            ...
            // 更新登录IP和时间
            if (StringUtils.equalsIgnoreCase(principal.getUserType(), "9")) {
                customerUserService.updateUserLoginInfo(user);
            } else {
                systemService.updateUserLoginInfo(user);
            }

            // 记录登录日志
            LogUtils.saveLog(ServletsUtils.getRequest(), "系统登录");
            return info;
        } else {
            return null;
        }
