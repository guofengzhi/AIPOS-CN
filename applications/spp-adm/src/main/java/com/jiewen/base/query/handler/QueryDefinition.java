...
package com.jiewen.base.query.handler;
...
import com.jiewen.base.query.domain.QueryContext;
import com.jiewen.jwp.common.utils.ConfigurationUtil;
        ...
        for (int i = 0; i < resources.length; i++) {
            ...
            try {
                ...
                do {
                    ...
                    if (previous != null) {
                        log.error("Duplicated Query register! id[{}],in file {}", query.getId(),
                                resource.toString());
                    }
                } while (true);
            ...
            synchronized (cachedFiles) {
                try {
                    if (resource.getFile().lastModified() > (cachedFiles.get(resource))
                        ...
                        List list = queryContext.getQueries();
                        Query query;
                        for (Iterator it = list.iterator(); it.hasNext(); log.debug(
                                "Update Query id[{}], in {}", query.getId(), resource.toString())) {
                            query = (Query) it.next();
                    ...
                    log.error("Fail to digester query from {}, reason:", resource.toString(), e);
                }
            }
