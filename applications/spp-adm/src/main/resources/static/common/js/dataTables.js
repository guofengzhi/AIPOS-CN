...
(function($, window, document, undefined) {
	...
	CommonTable.prototype.clearSearchDiv = function(selector) {
		...
		sel.find("select.select2").select2().val("");
	}
	...
	CommonTable.prototype.fnInitComplete = function(oSettings, json) {
		// 移动查询框的位置 与记录/页同行
		var _this = this;
		if ((!oSettings.oInit.lengthChange || !$('.col-sm-9:eq(0)',
				this.table.table().container()).html())
				&& !$("#" + this.searchDiv).hasClass("form-horizontal")) {
			...
			for (var j = 0; j < columns.length; j++) {
				...
				for (var i = 0; i < retData.pageInfo.list.length; i++) {
					...
					if (column.dateFormat) {
						eval(value_str
								+ "=formatDate(value, column.dateFormat)");
						value = eval(value_str);
					}
					// 格式化数字
					if (column.numberFormat) {
						eval(value_str
								+ "=$.number(value, column.numberFormat)");
						value = eval(value_str);
					}
					...
					if (column.render) {
						...
						if (value != null) {
							if (obj.type == "eq") {
								...
								var val = getDictLabel(obj.data, value, "无",
										true);
								eval(value_str + "=\"" + val + "\"");
								value = eval(value_str);
	...
	function gotoPage(tableId) {
		...
		var template = $("<li class='paginate_button active' style='float:left;margin-right:3px;'>"
				+ "   <div class='input-group'>"
				+ "       <span class='input-group-addon' style='padding:0px 8px;background-color:#fafafa;font-size: 12px;'>"+jQuery.i18n.prop('common.table.jumppage')+"</span>"
