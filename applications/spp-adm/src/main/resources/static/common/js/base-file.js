...
(function ($, window, document, undefined) {
    ...
    var BaseFile = function (element, options) {
        ...
        options.container = options.container || this.element.selector?"":this.element.selector.replace("#", "");
        //初始化文件图标信息
    ...
    BaseFile.prototype.default = {
        winId: "fileWin",
        width: 900,
        title: $.i18n.prop("common.file.upload"),
        //通用文件上传界面
        url: basePath + "/file/uploader",
        //文件回显图片
        showUrl:basePath+"/file/getFiles",
        //默认支持多文件上传
        multiple: true,
        //默认弹出附件上传窗口
        window:true,
    ...
    BaseFile.prototype.showFiles=function(options){
        options=options||{};
        if(!this.hasDisplayZone()){
            modals.error($.i18n.prop("common.sys.file.set.config.error"));
            return;
        }
        ...
        var fileResult,preview,previewConfig;
        if(fileIds){
        	fileResult = this.getFileResult(fileIds)
        	preview=fileResult.initialPreview;
            previewConfig=fileResult.initialPreviewConfig;
        }
        var self=this;
        ...
        if(!config){
            modals.error($.i18n.prop("common.sys.file.not.config"));
            return;
        }
    ...
    BaseFile.prototype.deleteFileIds=function(fileIds,targetIds){
        ...
        //没有文件删除，其中必有蹊跷
        if(!targetIds){
            modals.error($.i18n.prop("common.sys.file.not.delete.tips"));
            return;
        }
    ...
    BaseFile.prototype.initFileIds=function(){
        ...
        if(!this.options.window){
            if(!this.options.fileIdContainer||!$(this.options.fileIdContainer)){
                modals.info($.i18n.prop("common.sys.file.set.config.info"));
                return;
            }
        }
    ...
    BaseFile.prototype.getFileResult=function(fileIds){
        var ret=null;
        ajaxPost(this.options.showUrl,{fileIds:fileIds},function(result){
            ret=result;
        });
        return ret;
    };
